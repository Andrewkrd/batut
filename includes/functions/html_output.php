<?php
/*
  $Id: html_output.php,v 1.6 2012/09/09 20:15:14 ujirafika.ujirafika Exp $
 ...
 */
  ...
  function osc_href_link($page = null, $parameters = null, $connection = 'NONSSL', $add_session_id = false, $search_engine_safe = true, $use_full_address = false) {
    global $request_type, $osC_Session, $osC_Services, $osC_Database, $osC_Language;
    ...
    // NEW URL
    if(strpos($_SERVER["PHP_SELF"], "/admin/index.php") !== false)
		$link .= mb_strtolower($page);
    else
		$link .= $page;
	...
	if( !empty($cPath))	{
		...
		if(strpos($cPath, "_")  !== false)	{
				...
				$Qurl = $osC_Database->query('select category_url from :table_categories_description where categories_id in (:categories_id) and language_id = :language_id order by categories_id');			
				$Qurl->bindTable(':categories_id', implode(',', $cPath_array));
				$Qurl->setCache("osc_category_url_" . implode(',', $cPath_array), 21600);
		}
		else {
				$cPath = (int)$cPath;
				$Qurl = $osC_Database->query('select category_url from :table_categories_description where categories_id=(:categories_id) and language_id = :language_id order by categories_id');			
				$Qurl->bindInt(':categories_id', $cPath);
				$Qurl->setCache("osc_category_url_" . $cPath, 21600);
		}
		
        $Qurl->bindTable(':table_categories_description', TABLE_CATEGORIES_DESCRIPTION);       
        $Qurl->bindInt(':language_id', $osC_Language->getID());
        $Qurl->execute();
   
        while ($Qurl->next())	{
        	if(strpos($link, $Qurl->Value('category_url'))  == false) {
				$link .= "/" . $Qurl->Value('category_url');
        	}
        }
...
// Add the session ID when moving from different HTTP and HTTPS servers, or when SID is defined
  ...
  /*  while (strstr($link, '&&')) {
   ...
   /* if ( ($search_engine_safe === true) && isset($osC_Services) && $osC_Services->isStarted('sefu')) {
	...
	// Добавить слэш перед images если его нет
	if(strpos(substr($link, 0, 10), "images/")  !== false)	{
		if(substr($link, 0, 1) !== "/") {
			$link = "/" . $link;
		}
	}		
 ...
 */
  ...
  function osc_link_object($link, $object, $parameters = null) {
  	if(strpos($_SERVER["PHP_SELF"], "/admin/index.php") !== false)
    	...
    	return '<a href="' . mb_strtolower($link) . '"' . (!empty($parameters) ? ' ' . $parameters : '') . '>' . $object . '</a>';
  }
 ...
 */
  ...
  function osc_image($image, $title = null, $width = 0, $height = 0, $parameters = null) {
    ...
    if(substr($image, 0, 1) !== "/" && strpos($_SERVER["REQUEST_URI"], "/admin") === false)
    	$image = "/" . $image;
 ...
 */
  ...
  function osc_draw_button_dropdown_menu($name, $values, $default = null, $parameters = null) {

  	$url_param = array("filter" => "-1", "sort" => "-1");
  	
	foreach($_GET as $key=>$val)	{		
		if($key == 'filter')
			$url_param["filter"] = intval($_GET['filter']);
		if($key == 'sort')
			$url_param["sort"] = intval($_GET['sort']);
	}
	
	if(strpos($_SERVER['REQUEST_URI'], "?") !== false)
		$url_str = substr($_SERVER['REQUEST_URI'], 0, strpos($_SERVER['REQUEST_URI'], "?"));
	else
	...
	$default_str = false;	
	for ($i=0, $n=sizeof($values); $i<$n; $i++) {
	if ( (!is_null($default) && !is_array($default) && ((string)$default == (string)$values[$i]['id']))  )
        foreach($values as $val)
        	if((string)$val['id'] == (string)$default)
        		$default_str = $val['text'];
	if(!$default_str)
		$default_str = "Все производители";
	}
	...
	// default
    $field .= '<button class="btn dropdown-toggle" data-toggle="dropdown">' . $default_str . '<span class="caret"></span></button>';
                
	$field .= '<ul class="dropdown-menu">';
    
	for ($i=0, $n=sizeof($values); $i<$n; $i++) {
		$sort = false;
		$filter = false;

		if($parameters == 'sort' && $url_param["filter"] > 0)
			$filter = "filter=" . $url_param["filter"];
		if($parameters == 'filter' && $url_param["sort"] > 0)
			$sort = "sort=" . $url_param["sort"];
				
		if($parameters == "filter" && osc_output_string($values[$i]['id'] > 0))
			$filter = $parameters . '=' . osc_output_string($values[$i]['id']);
		if($parameters == "sort" && osc_output_string($values[$i]['id'] > 0))
			$sort = $parameters . '=' . osc_output_string($values[$i]['id']);
		
		if($sort && $filter)
			$param_url = "?" . $sort . "&" . $filter;
		elseif(!$sort && $filter)
			$param_url = "?" . $filter;
		if($sort && !$filter)
			$param_url = "?" . $sort;
		...
		$field .= '<li><a href=" ' . $url_str . $param_url . '">' . osc_output_string($values[$i]['text'], array('"' => '&quot;', '\'' => '&#039;', '<' => '&lt;', '>' => '&gt;')) . '</a></li>';
	}
 ...
 */
  ...
  function osc_draw_label($text, $for, $access_key = null, $required = false, $class = false) {
    if (!is_bool($required)) {
      $required = false;
    }

    return '<label for="' . osc_output_string($for) . '"' . (!empty($access_key) ? ' accesskey="' . osc_output_string($access_key) . '"' : '') . (!empty($class) ? ' class="' . osc_output_string($class) . '"' : '') . '>' . osc_output_string($text) . ($required === true ? '<em>*</em>' : '') . '</label>';
  }

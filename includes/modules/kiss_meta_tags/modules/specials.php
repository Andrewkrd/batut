...
* @Id $Id: specials.php,v 1.2 2012/01/30 20:23:22 ujirafika.ujirafika Exp $:  Full Details   
*/
  ...
  final class KissMT_Module extends KissMT_Modules {
    ...
    public function process() {
      ...
      $this->cache_name = $this->setCacheString( __FILE__, 'listing', 'specials' . $this->cache_suffix);
      if ( false !== $this->retrieve( $this->cache_name ) ) {
        KissMT::init()->setCanonical( $this->checkCanonical('',$page_string) );
        return;
      } 

		$res = osC_Specials::getListing();
      ...
      KissMT::init()->setCanonical( $this->checkCanonical('',$page_string) ); 
      $this->parse( KissMT::init()->entities( sprintf( KISSMT_SPECIALS_TEXT, $leading_values ), $decode = true ), KissMT::init()->entities( $list_string, $decode = true ),null,null,$list_string_first,$metakeywords);

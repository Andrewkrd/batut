<?php
/*
  $Id: payment.php,v 1.6 2012/01/08 21:11:38 ujirafika.ujirafika Exp $
...
// class constructor
    function osC_Payment($module = '') {
      ...
      $adr = $osC_ShoppingCart->getBillingAddress();

      while ($Qmodules->next()) {
      	
      	
      /*if((strpos(trim($adr["city"]), "нодар") && $Qmodules->value('code') == "manager") ||  ( strpos(trim($adr["city"]), "нодар") && $Qmodules->value('code') == "cod"))
         {}
      else     */  	
      ...
      if (empty($this->_modules) === false) {
        ...
        foreach ($this->_modules as $modules) {
          ...
          /*
          if((strpos(trim($adr["city"]), "нодар") && $modules == "manager") || ( strpos(trim($adr["city"]), "нодар") && $Qmodules->value('code') == "cod"))
          { echo 111;}
          else {     */    	
          	$module_class = 'osC_Payment_' . $modules;
			//echo trim(mb_strtolower($adr["city"], 'utf-8'));

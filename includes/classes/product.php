<?php
/*
  $Id: product.php,v 1.10 2012/09/28 19:11:08 ujirafika.ujirafika Exp $
...
*/
  ...
  class osC_Product {
    ...
    function __construct($id) {
      ...
      $man_sef = false;
      $location = false;
            if(strpos($_SERVER["REQUEST_URI"], "manufacturers,") > 0 && $_SERVER["SCRIPT_NAME"] == "/products.php"){
          	$chpos = strrpos($_SERVER["REQUEST_URI"], "manufacturers,");          	
          	$man_id = substr($_SERVER["REQUEST_URI"], $chpos + 14);

          	if($man_id == "0" || $man_id == "")	{
          		$product_sef = substr($url, strpos($url, "/products.php/") + 14, strpos($url, "manufacturers,") - 15 );
				$Qman = $osC_Database->query('select m.manufacturers_sef from :table_manufacturers as m, :table_products as p, :table_products_description as pd where pd.products_keyword = :products_sef AND pd.products_id = p.products_id AND p.manufacturers_id = m.manufacturers_id');
				$Qman->bindTable(':table_manufacturers', TABLE_MANUFACTURERS);
				$Qman->bindTable(':table_products', TABLE_PRODUCTS);
				$Qman->bindTable(':table_products_description', TABLE_PRODUCTS_DESCRIPTION);
				$Qman->bindValue(':products_sef', $product_sef);
				$Qman->execute();				
	          	$man_sef = $Qman->value('manufacturers_sef');
          	}

          	if(is_numeric($man_id) && $man_id !== "0"  ){
          	...
          	if($man_sef) {
          		echo $location= substr($_SERVER["REQUEST_URI"], 0, $chpos + 14) . $man_sef;
	          	
	          	/*if(!apc_fetch($_SERVER["REQUEST_URI"] . " > " . $location))
        			apc_add($_SERVER["REQUEST_URI"] . " > " . $location, $_SERVER);*/
      ...
      if ( !empty($id) ) {
        if ( is_numeric($id) ) {
          ...
          if ( $Qproduct->numberOfRows() === 1 ) {
            ...
            if ( !empty($this->_data) ) {
              $Qdesc = $osC_Database->query('select products_name as name, products_description as description, products_keyword as keyword, products_tags as tags, products_url as url, products_subtitle as subtitle from :table_products_description where products_id = :products_id and language_id = :language_id');
          ...
          $Qproduct = $osC_Database->query('select p.products_id as id, p.parent_id, p.products_quantity as quantity, p.products_price as price, p.products_model as model, p.products_tax_class_id as tax_class_id, p.products_weight as weight, p.products_weight_class as weight_class_id, p.products_date_added as date_added, p.manufacturers_id, p.has_children, p.available, pd.products_name as name, pd.products_description as description, pd.products_param as param, pd.products_keyword as keyword, pd.products_tags as tags, pd.products_url as url from :table_products p, :table_products_description pd where pd.products_keyword = :products_keyword and pd.language_id = :language_id and pd.products_id = p.products_id and p.products_status = :products_status');
          $Qproduct->bindTable(':table_products', TABLE_PRODUCTS);
    ...
    function getPriceFormated($with_special = false) {
      ...
      if ($with_special && $osC_Services->isStarted('specials') && ($new_price = $osC_Specials->getPrice($this->_data['id']))) {

        $price = '<s>' . $osC_Currencies->displayPrice($this->getClearMinPrice(), $this->_data['tax_class_id']) . '</s><span class="productSpecialPrice"> ' . $osC_Currencies->displayPrice($new_price, $this->_data['tax_class_id']) . '</span>';
      } else { // Не выводить надпись "Цена от" даже если есть варианты
        if ( $this->hasVariants() ) {
 	 ...
 	 function getPriceFormatedList($with_special = false) {
      global $osC_Services, $osC_Specials, $osC_Currencies, $osC_Language;

      if ($with_special && $osC_Services->isStarted('specials') && ($new_price = $osC_Specials->getPrice($this->_data['id']))) {

        $price = '<span class="label label-important price pull-right"><s style="font-weight: normal; margin: 5px;">' . $osC_Currencies->displayPrice($this->getClearMinPrice(), $this->_data['tax_class_id']) . '</s><br /><span class="productSpecialPrice"> ' . $osC_Currencies->displayPrice($new_price, $this->_data['tax_class_id']) . '</span></span>';
      } else { // Не выводить надпись "Цена от" даже если есть варианты
        if ( $this->hasVariants() ) {
          $price =  /*$osC_Language->get('variants_tag') . ' '.*/ $this->getVariantMinPrice();
        } else {
          $price = '<span class="label label-info price pull-right">' . $osC_Currencies->displayPrice($this->_data['price'], $this->_data['tax_class_id']) . '</span>';
        }
      }
...
$price = $osC_Currencies->displayPrice($price, $tax);
   ...
   function getClearMinPrice() {
    ...
    function getManufacturer($seo=true) {
      ...
      if($seo)
      	...
      	return $osC_Manufacturer->getTitle(false);
    }

<?php
/*
  $Id: order.php,v 1.3 2012/02/05 20:42:40 ujirafika.ujirafika Exp $
...
// Update products_ordered (for bestsellers list)
    ...
    function sendEmail($id) {
      global $osC_Database, $osC_Language, $osC_Currencies, $osC_Customer;

      $fast_order = false;
      if($osC_Customer->getID() == 2)
		$fast_order = true; 
      
//START TEXT-PLAIN E-MAIL	
	  if (EMAIL_USE_HTML == '-1' && !$fast_order && 0 == 1) {
...
// Если был оформлен быстрый заказ состовляем письо по особому
      if ($fast_order) {
      $Qorder = $osC_Database->query('select * from :table_orders where orders_id = :orders_id limit 1');
      $Qorder->bindTable(':table_orders', TABLE_ORDERS);
      $Qorder->bindInt(':orders_id', $id);
      ...
      if ($Qorder->numberOfRows() === 1) {
        ...
        while ($Qtotals->next()) { if(strpos($Qtotals->value('title'), "Итого") !== false) {
          $email_order .= strip_tags($Qtotals->value('title') . ' ' . $Qtotals->value('text')) . "<br>";
        } }
        ...
        while ($Qstatuses->next()) {
          if( strpos($Qstatuses->value('comments'), "Подписка") == true  )
          	$comment = substr($Qstatuses->value('comments'), 0, strpos($Qstatuses->value('comments'), "Подписка"));
          else
          	$comment = $Qstatuses->value('comments');
          $email_order .= "Детали заказа: <br>" . wordwrap(str_replace("<br>", "<br>", $comment), 60, "<br>", 1) . "<br><br>";
        }

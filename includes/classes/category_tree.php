<?php
/*
  $Id: category_tree.php,v 1.3 2012/02/12 20:22:19 ujirafika.ujirafika Exp $
 ...
 */
    ...
    public function __construct() {
      ...
      if ( $osC_Cache->read('category_tree-' . $osC_Language->getCode(), 720) ) {
        ...
        $Qcategories = $osC_Database->query('select c.categories_id, c.parent_id, c.categories_image, cd.categories_name, cd.category_url from :table_categories c, :table_categories_description cd where c.categories_id = cd.categories_id and cd.language_id = :language_id order by c.parent_id, c.sort_order, cd.categories_name');
        ...
        while ( $Qcategories->next() ) {
          $this->_data[$Qcategories->valueInt('parent_id')][$Qcategories->valueInt('categories_id')] = array('name' => $Qcategories->value('categories_name'),
                                                                                                             ...
                                                                                                             'count' => 0,
          																									 'category_url' => $Qcategories->value('category_url'));
        }
    ...
    function reset() {
      ...
      $this->parent_start_string = '<strong>';
      $this->parent_end_string = '</strong>';
 ...
 */
      ...
      if ( isset($this->_data[$parent_id]) ) {
        ...
        if($category['count']>0){
		 ...
		 if (SERVICES_CATEGORY_PATH_CALCULATE_PRODUCT_COUNT == '1') {
          ...
          $result .= str_repeat($this->spacer_string, $this->spacer_multiplier * $level) . osc_link_object(osc_href_link("category", 'cPath=' . $category_link), $link_title);
   ...
   /*
    * Массив со ссылками на категории для карты сайта
    */ 
    function buildBranchMap($parent_id, $level = 0) {
      ...
      if ( isset($this->_data[$parent_id]) ) {
        ...
        if($category['count']>0){
		 ...
		 if (SERVICES_CATEGORY_PATH_CALCULATE_PRODUCT_COUNT == '1') {
          ...
          $result[$category_id] = array('link' => str_repeat($this->spacer_string, $this->spacer_multiplier * $level) . osc_href_link(FILENAME_DEFAULT, 'cPath=' . $category_link));
          ...
          if ( isset($this->_data[$category_id]) && (($this->max_level == '0') || ($this->max_level > $level+1)) ) {
            if ( $this->follow_cpath === true ) {
              if ( in_array($category_id, $this->cpath_array) ) {
                $result[$category_id] = $this->buildBranchMap($category_id, $level+1);
              }
            } else {
              $result[$category_id] = $this->buildBranchMap($category_id, $level+1);
            }
        ...
        foreach ($categories as $category_id => $info) {
          if ($id == $category_id) {
            return array('id' => $id,
                         ...
                         'image' => $info['image'],
                         'count' => $info['count'],
            			 'category_url' => $info['category_url']
                        );
          }
        }
      }

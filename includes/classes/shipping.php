<?php
/*
  $Id: shipping.php,v 1.8 2011/12/18 16:31:44 ujirafika.ujirafika Exp $
...
// class methods
    ...
    function _calculate() {
      ...
      if (is_array($this->_modules)) {
        ...
        $adr = $osC_ShoppingCart->getShippingAddress();
        if(!isset($adr["city"]))
        	$adr["city"] = "";

		// Бесплатную доставку нужно оставить в качестве альтерантивы для жителей Краснодара 
       // if (defined('MODULE_SHIPPING_FREE_STATUS') && isset($GLOBALS['osC_Shipping_free']) && $GLOBALS['osC_Shipping_free']->isEnabled()) {
       	if (defined('MODULE_SHIPPING_FREE_STATUS') && isset($GLOBALS['osC_Shipping_free']) && $GLOBALS['osC_Shipping_free']->isEnabled() && strpos(trim($adr["city"]), "нодар")){
          $include_quotes[] = 'osC_Shipping_free';
          if(isset($GLOBALS['osC_Shipping_item']))
         	 $include_quotes[] = 'osC_Shipping_item';
        } else {
          foreach ($this->_modules as $module) {
            if ($GLOBALS['osC_Shipping_' . $module]->isEnabled() && $module !== "free" ) {
            	if(strpos(trim($adr["city"]), "нодар") && $module == "table") 
            	{}
            	else            	

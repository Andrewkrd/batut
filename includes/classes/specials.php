<?php
/*
  $Id: specials.php,v 1.1 2011/08/30 21:06:02 ujirafika.ujirafika Exp $
...
/* Public methods */
    ...
    function &getListing() {
      ...
      $Qspecials = $osC_Database->query('select p.products_id, p.products_price, p.products_tax_class_id, pd.products_name, pd.products_keyword, s.specials_new_products_price, i.image from :table_products p left join :table_products_images i on (p.products_id = i.products_id and i.default_flag = :default_flag), :table_products_description pd, :table_specials s where p.products_status = 1 and s.products_id = p.products_id and p.products_id = pd.products_id and pd.language_id = :language_id and s.status = 1');
	// group by s.specials_date_added desc      с этим отображалась только одна запись

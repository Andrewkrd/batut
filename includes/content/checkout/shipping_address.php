<?php
/*
  $Id: shipping_address.php,v 1.5 2012/01/08 20:31:30 ujirafika.ujirafika Exp $
...
// if no shipping destination address was selected, use their own address as default
    ...
    function &getListing() {
      ...
      $Qaddresses = $osC_Database->query('select ab.address_book_id, ab.entry_firstname as firstname,
      ab.entry_lastname as lastname, ab.entry_company as company, 
      ab.entry_street_address as street_address, ab.entry_suburb as suburb, 
      ab.entry_city as city, ab.entry_postcode as postcode, ab.entry_state as state,
       ab.entry_zone_id as zone_id, ab.entry_country_id as country_id
        from :table_address_book ab where ab.customers_id = :customers_id');
      ...
      if (($osC_Customer->hasDefaultAddress() === false) || (!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['street_address'])) ) {
        ...
        if (ACCOUNT_STATE > 0) {
          ...
          if ($entry_state_has_zones === true) {
            ...
            if ($Qzone->numberOfRows() === 1) {
              $zone_id = 0;//$Qzone->valueInt('zone_id');
            } else {
       ...
       /* if ( (is_numeric($_POST['country']) === false) || ($_POST['country'] < 1) ) {
        ...
        if ($osC_MessageStack->size('checkout_address') === 0) {
          ...
          $Qab->bindValue(':entry_state', '');
          $Qab->bindInt(':entry_country_id', "0");
          $Qab->bindInt(':entry_zone_id', '0');
          ...
          if ($Qab->affectedRows() === 1) {
            ...
            if ($osC_Customer->hasDefaultAddress() === false) {
              ...
              $osC_Customer->setCountryID(0);
              $osC_Customer->setZoneID(0);

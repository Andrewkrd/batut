<?php
/*
  $Id: index.php,v 1.4 2012/07/31 22:07:35 ujirafika.ujirafika Exp $
    ...
    function osC_Index_Index() {
      ...
      if (isset($cPath) && (empty($cPath) === false)) {
        if ($osC_Services->isStarted('breadcrumb')) {
          $Qcategories = $osC_Database->query('select categories_id, categories_name, category_url from :table_categories_description where categories_id in (:categories_id) and language_id = :language_id');
          ...
          while ($Qcategories->next()) {
            ...
            $cat_url_after .= "/" . $Qcategories->value('category_url');
          }

          $Qcategories->freeResult();

          for ($i=0, $n=sizeof($cPath_array); $i<$n; $i++) {
            $osC_Breadcrumb->add($categories[$cPath_array[$i]], osc_href_link("category", 'cPath=' . implode('_', array_slice($cPath_array, 0, ($i+1)))));
          }
        }
...
/* Private methods */
    ...
    function _process() {
      ...
      if (isset($_GET['sort']) && !empty($_GET['sort'])) {
      	if($_GET['sort'] == 0)
      		$osC_Products->setSortBy("name");
      	if($_GET['sort'] == 1)
      		$osC_Products->setSortBy("name", '-');
      	if($_GET['sort'] == 2)
      		$osC_Products->setSortBy("price");
      	if($_GET['sort'] == 3)
      		$osC_Products->setSortBy("price", '-');

<?php
/*
  $Id: products.php,v 1.3 2013/02/24 16:14:59 ujirafika.ujirafika Exp $
...
*/
  ...
  class osC_Products_Admin {
    ...
    public static function find($search, $category_id = null, $pageset = 1) {
      ...
      $Qproducts->appendQuery('and (pd.products_name like :products_name or pd.products_keyword like :products_keyword or p.products_model like :products_model) order by pd.products_name');
      $Qproducts->bindTable(':table_products', TABLE_PRODUCTS);
      $Qproducts->bindTable(':table_products_description', TABLE_PRODUCTS_DESCRIPTION);
      $Qproducts->bindInt(':language_id', $osC_Language->getID());
      $Qproducts->bindValue(':products_name', '%' . $search . '%');
      $Qproducts->bindValue(':products_keyword', '%' . $search . '%');
      $Qproducts->bindValue(':products_model', '%' . $search . '%');
    ...
    public static function save($id = null, $data) {
      ...
      $Qproduct->bindInt(':available', $data['available']);

/*      foreach ($osC_Language->getAll() as $l) {
      	$Qproduct->bindValue(':products_subtitle', $data['products_subtitle'][$l['id']]);
      	$Qproduct->bindValue(':products_param', $data['products_param'][$l['id']]);
      }*/
      ...
      if ( $error === false ) {
        foreach ($osC_Language->getAll() as $l) {
          ...
          $Qpd->bindValue(':products_url', $data['products_url'][$l['id']]);
			...
			$Qpd->bindValue(':products_subtitle', $data['products_subtitle'][$l['id']]);
			$Qpd->bindValue(':products_param', $data['products_param'][$l['id']]);

<?php
/*
  $Id: edit.php,v 1.3 2012/04/16 20:05:40 ujirafika.ujirafika Exp $
...
tinyMCE.init({
  ...
  cleanup : false,
		...
		theme_advanced_buttons3 : "spellchecker,|,tablecontrols,|,hr,removeformat,visualaid,|,sub,sup,|,charmap,emotions,iespell,media,advhr,|,print,|,ltr,rtl",
...
<?php
  if ( is_numeric($_GET[$osC_Template->getModule()]) ) {
    ...
    $Qpd = $osC_Database->query('select products_name, products_description, products_keyword, products_tags, products_url, language_id, products_subtitle, products_param from :table_products_description where products_id = :products_id');
    $Qpd->bindTable(':table_products_description', TABLE_PRODUCTS_DESCRIPTION);
    $Qpd->bindInt(':products_id', $osC_ObjectInfo->getInt('products_id'));
    $Qpd->execute();
    
    
    $Qpd1 = $osC_Database->query('select available from :table_products where products_id = :products_id');
    $Qpd1->bindTable(':table_products', TABLE_PRODUCTS);
    $Qpd1->bindInt(':products_id', $osC_ObjectInfo->getInt('products_id'));
    $Qpd1->execute();
    ...
    while ($Qpd->next()) {
      ...
      $available = $Qpd1->valueInt('available');
      
      $products_subtitle[$Qpd->valueInt('language_id')] = $Qpd->value('products_subtitle');
      $products_param[$Qpd->valueInt('language_id')] = $Qpd->value('products_param');
    }
  }
      ...
      <div id="languageTabs_<?php echo $l['code']; ?>">
        <fieldset>
          ...
          <div><label for="<?php echo 'products_url[' . $l['id'] . ']'; ?>"><?php echo $osC_Language->get('field_url'); ?></label><?php echo osc_draw_input_field('products_url[' . $l['id'] . ']', (isset($osC_ObjectInfo) && isset($products_url[$l['id']]) ? $products_url[$l['id']] : null)); ?></div>
           ...
           <div><label for="<?php echo 'products_subtitle[' . $l['id'] . ']'; ?>">Анонс</label><?php echo osc_draw_textarea_field('products_subtitle[' . $l['id'] . ']', (isset($osC_ObjectInfo) && isset($products_subtitle[$l['id']]) ? $products_subtitle[$l['id']] : null)); ?></div>
           <div><label for="<?php echo 'products_param[' . $l['id'] . ']'; ?>">Характеристики</label><?php echo osc_draw_textarea_field('products_param[' . $l['id'] . ']', (isset($osC_ObjectInfo) && isset($products_param[$l['id']]) ? $products_param[$l['id']] : null)); ?></div>
        </fieldset>
      </div>

<?php
/*
  $Id: cms.php,v 1.2 2013/02/22 20:42:14 ujirafika.ujirafika Exp $
  ...
  class osC_cms_Admin {
    ...
    public static function save($id = null, $data, $default = false) {
      ...
      foreach ( $osC_Language->getAll() as $l ) {
        if ( is_numeric($id) ) {
          $Qstatus = $osC_Database->query('update :table_cms set active = :active, cms_name = :cms_name, cms_description = :cms_description, last_modified = :last_modified, cms_short_text = :cms_short_text, cms_url = :cms_url, cms_keyword = :cms_keyword where cms_id = :cms_id and language_id = :language_id');
        } else {
          $Qstatus = $osC_Database->query('insert into :table_cms (cms_id, language_id, cms_name, cms_description, active, last_modified, date_added, cms_short_text, cms_url, cms_keyword) values (:cms_id, :language_id, :cms_name, :cms_description, :active, :last_modified, :date_added, :cms_short_text, :cms_url, :cms_keyword)');
        }
		...
		$Qstatus->bindValue(':cms_keyword', $data['cms_keyword'][$l['id']]);
		...
		$cms_url = strtr($data['cms_name'][$l['id']],$Atr);    
		$cms_url = preg_replace('/[^A-Za-z0-9_\-]/', '', $cms_url);

		$Qstatus->bindValue(':cms_url', $cms_url);

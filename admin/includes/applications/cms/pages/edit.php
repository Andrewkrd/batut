<?php
/*
  $Id: edit.php,v 1.1 2011/08/29 22:03:04 ujirafika.ujirafika Exp $
...
<?php
  if ( isset($_GET['pID']) ) {
    ...
    $Qpd = $osC_Database->query('select cms_name, cms_description, cms_short_text, language_id, cms_keyword from :table_cms where cms_id = :cms_id');
    ...
    while ($Qpd->next()) {
      ...
      $cms_keyword[$Qpd->valueInt('language_id')] = $Qpd->value('cms_keyword');
    }
  }
...
?>
      ...
      <div id="languageTabs_<?php echo $l['code']; ?>">
        <fieldset>
          ...
          <div><label for="<?php echo 'cms_keyword[' . $l['id'] . ']'; ?>">Ключевые слова</label><?php echo osc_draw_input_field('cms_keyword[' . $l['id'] . ']', (isset($osC_ObjectInfo) && isset($cms_keyword[$l['id']]) ? $cms_keyword[$l['id']] : null)); ?></div>
        </fieldset>
      </div>
